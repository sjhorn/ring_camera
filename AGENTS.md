...
## Project Overview
This repository is for the Dart package **`ring_camera`** flutter package and example. This is a companion flutter package to ../ring_client_api dart package that we depend on and has the majority of the api functionality. The original typescript source we ported frmo is in in ../ring_client_api/ring/packages/ring-client-api we will refer to it as dgrief. ring_camera only implements the parts of the ring_client_api that rely on webrtc and all the functionality that can remain in the dart package should go there.  
Our focus will be porting each file needed for for webrtc to match the same name and also port the tests and examples to achieve exact input and output match prior to testing with the real ring API. The boilerplate from dart create -t package will be replaced with suitable dart files to complete the package. The README.md will follow the docs from ./ring/packages/ring-client-api with the webrtc focus and referring back to the dart ring-client-api package in ../ring-client-api as needed. The LICENCE will also match. 
...
For typescript types and json mapping be sure to address the snake_case to camelCase. The quirks have been documented in TYPESCRIPT_DIFFERENCES.md that we will keep up to date as we progress. Also we aim to use ../ring-client-api/ring/packages/ring-client-api for the source code rather than web calls to github.com.
...
- Package structure:
  - `lib/` — main public API  
  - `example/camera_viewer` — usage examples  
